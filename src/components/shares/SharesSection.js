...
const SharesSection = () => {
  ...
  const { loading, error, sendHttpRequest } = useGetRequest(
    `https://api.polygon.io/v2/aggs/grouped/locale/us/market/stocks/2023-01-09?adjusted=true&apiKey=Oe8nZqMTIZMk4frn47yOHHZ2SgnQPmh3`
  );
    ...
    <>
      {loading ? (
        ...
        <section className={styles.shares}>
          <h3>Курс акций</h3>
          <div className={styles["shares-cards"]}>{items}</div>
        </section>
      )}
    </>
  );
