...
const SliderSection = () => {
  ...
  const { loading, error, sendHttpRequest } = useGetRequest(
    `https://gnews.io/api/v4/top-headlines?category=world&apikey=caff4ea3f88849a0348df3f99a491fd2&lang=ru&category=nation`
  );
  ...
  return (
    <section hidden className={styles.slider}>
      {!isOpenedMenu && <Slider {...settings}>{items}</Slider>}
    </section>
  );
};
